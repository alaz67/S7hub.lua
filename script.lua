...
local S7Speed = {Enabled=false,SpeedUI=nil,SpeedValue=58,StealValue=31,HeartbeatConn=nil,character=nil,hrp=nil,hum=nil}
...
local function enableS7Speed()
    if S7Speed.Enabled then return end; S7Speed.Enabled=true
    if S7Speed.SpeedUI and S7Speed.SpeedUI.ToggleBtn then S7Speed.SpeedUI.ToggleBtn.Text="ON"; S7Speed.SpeedUI.Header.BackgroundColor3=ACCENT end
    if player.Character then S7Speed.character=player.Character; S7Speed.hrp=player.Character:WaitForChild("HumanoidRootPart"); S7Speed.hum=player.Character:WaitForChild("Humanoid") end
    player.CharacterAdded:Connect(function(char) S7Speed.character=char; S7Speed.hrp=char:WaitForChild("HumanoidRootPart"); S7Speed.hum=char:WaitForChild("Humanoid") end)
    S7Speed.HeartbeatConn=RunService.Heartbeat:Connect(function()
        ...
        if moveDir.Magnitude>0 then
            ...
            if targetSpeed and targetSpeed>0 then S7Speed.hrp.AssemblyLinearVelocity=Vector3.new(moveDir.X*targetSpeed,S7Speed.hrp.AssemblyLinearVelocity.Y,moveDir.Z*targetSpeed) end
        end
    end)
end

local function disableS7Speed()
    if not S7Speed.Enabled then return end; S7Speed.Enabled=false
    if S7Speed.SpeedUI and S7Speed.SpeedUI.ToggleBtn then S7Speed.SpeedUI.ToggleBtn.Text="OFF"; S7Speed.SpeedUI.Header.BackgroundColor3=Color3.fromRGB(30,25,45) end
...
local WPRight={{label="R1",pos=Vector3.new(-475.37,-7,26.63),color=Color3.new(0.313726,0.705882,1),offset=Vector3.new(0,0,0)},{label="R2",pos=Vector3.new(-486.76,-3.65,17.55),color=Color3.new(0.392157,1,0.470588),offset=Vector3.new(0,0,0)},{label="R3",pos=Vector3.new(-475.57,-5.62,31.04),color=Color3.new(1,0.862745,0.196078),offset=Vector3.new(0,0,0)},{label="R4",pos=Vector3.new(-476.09,-6.56,97.06),color=Color3.new(1,0.313726,0.627451),offset=Vector3.new(0,0,0)}}
local WPLeft={{label="L1",pos=Vector3.new(-476.48,-6.28,92.73),color=Color3.new(0.313726,0.705882,1),offset=Vector3.new(0,0,0)},{label="L2",pos=Vector3.new(-483.12,-4.95,94.80),color=Color3.new(0.392157,1,0.470588),offset=Vector3.new(0,0,0)},{label="L3",pos=Vector3.new(-473.38,-8.40,22.34),color=Color3.new(1,0.862745,0.196078),offset=Vector3.new(0,0,0)},{label="L4",pos=Vector3.new(-483.12,-4.95,94.80),color=Color3.new(1,0.313726,0.627451),offset=Vector3.new(0,0,0)}}
local function wpPos(wp) return wp.pos+wp.offset end
local function getL1() return wpPos(WPLeft[1]) end; local function getLEND() return wpPos(WPLeft[2]) end; local function getLFINAL() return wpPos(WPLeft[3]) end
local function getR1() return wpPos(WPRight[1]) end; local function getREND() return wpPos(WPRight[2]) end; local function getRFINAL() return wpPos(WPRight[4]) end
...
local TPA=Vector3.new(-472.60,-7.00,57.52); local TPRB=Vector3.new(-471.76,-7.00,26.22); local TPR1=Vector3.new(-483.51,-5.10,18.89)
...
batAimbotEnabled=false; BAT_ENGAGE_RANGE=5; AIMBOT_SPEED=60
...
floatEnabled=false; floatConn=nil; autoPlayAfterTP=false; medusaCounterEnabled=false
noCamCollisionEnabled=false; shinyGraphicsEnabled=false; removeAccessoriesEnabled=false
    ...
    pcall(function()
        if readfile and isfile and isfile(CONFIG_KEY..".json") then
            ...
            if ok and d then
                ...
                for k,_ in pairs(Keybinds) do if d["KEY_"..k] then pcall(function() Keybinds[k]=Enum.KeyCode[d["KEY_"..k]] end) end end
...
local function hookTPCharacter(char) if tpStateConn then tpStateConn:Disconnect(); tpStateConn=nil end; if tpChildConn then tpChildConn:Disconnect(); tpChildConn=nil end; if tpChildRemConn then tpChildRemConn:Disconnect(); tpChildRemConn=nil end; local hum=char:WaitForChild("Humanoid"); local spawnTime=tick(); local function isSpawnGrace() return (tick()-spawnTime)<2 end; tpStateConn=hum.StateChanged:Connect(function(_,ns) if not RagdollTPEnabled then return end; if isSpawnGrace() then tpWasRagdolled=false; return end; if TP_RAGDOLL_STATES[ns] then if not tpWasRagdolled then tpWasRagdolled=true; task.defer(doRagdollTP) end else tpWasRagdolled=false end end); tpChildConn=char.ChildAdded:Connect(function(child) if not RagdollTPEnabled then return end; if isSpawnGrace() then return end; if isMedusaTPChild(child.Name) then if not tpMedusaActive then tpMedusaActive=true; task.defer(doMedusaTP) end; return end; if child.Name=="Ragdoll" or child.Name=="IsRagdoll" then if not tpWasRagdolled then tpWasRagdolled=true; task.defer(doRagdollTP) end end end); tpChildRemConn=char.ChildRemoved:Connect(function(child) if child.Name=="Ragdoll" or child.Name=="IsRagdoll" then tpWasRagdolled=false end; if isMedusaTPChild(child.Name) then tpMedusaActive=false end end) end
...
local function scanForCountdown() local best=-1; for _,gui in ipairs(player.PlayerGui:GetChildren()) do if gui:IsA("ScreenGui") and gui.Name~="S7_GUI" and gui.Name~="S7_OpenClose" and gui.Name~="S7_FloatingTitle" then for _,desc in ipairs(gui:GetDescendants()) do if desc:IsA("TextLabel") and desc.Visible then local n=tonumber(desc.Text); if n and n==math.floor(n) and n>=1 and n<=15 then local abs=desc.AbsoluteSize; if abs.X>40 and abs.Y>30 and n>best then best=n end end end end end end; return best end
...
-- MAIN GUI
local ScreenGui=Instance.new("ScreenGui"); ScreenGui.Name="S7_GUI"; ScreenGui.ResetOnSpawn=false; ScreenGui.ZIndexBehavior=Enum.ZIndexBehavior.Sibling; ScreenGui.Parent=player:WaitForChild("PlayerGui")
...
do local guiScale=1.0; local BASE_W,BASE_H=360,650; local plusBtn=Instance.new("TextButton",TitleBar); plusBtn.Size=UDim2.new(0,26,0,26); plusBtn.Position=UDim2.new(1,-58,0.5,-13); plusBtn.BackgroundColor3=Color3.fromRGB(25,15,40); plusBtn.Text="+"; plusBtn.Font=Enum.Font.GothamBlack; plusBtn.TextSize=16; plusBtn.TextColor3=ACCENT; plusBtn.BorderSizePixel=0; Instance.new("UICorner",plusBtn).CornerRadius=UDim.new(0,6); local minusBtn=Instance.new("TextButton",TitleBar); minusBtn.Size=UDim2.new(0,26,0,26); minusBtn.Position=UDim2.new(1,-28,0.5,-13); minusBtn.BackgroundColor3=Color3.fromRGB(25,15,40); minusBtn.Text="-"; minusBtn.Font=Enum.Font.GothamBlack; minusBtn.TextSize=16; minusBtn.TextColor3=ACCENT; minusBtn.BorderSizePixel=0; Instance.new("UICorner",minusBtn).CornerRadius=UDim.new(0,6); plusBtn.MouseButton1Click:Connect(function() guiScale=math.min(guiScale+0.1,2.0); local w=math.floor(BASE_W*guiScale); local h=math.floor(BASE_H*guiScale); MainFrame.Size=UDim2.new(0,w,0,h); MainFrame.Position=UDim2.new(0.5,-w/2,0.5,-h/2) end); minusBtn.MouseButton1Click:Connect(function() guiScale=math.max(guiScale-0.1,0.4); local w=math.floor(BASE_W*guiScale); local h=math.floor(BASE_H*guiScale); MainFrame.Size=UDim2.new(0,w,0,h); MainFrame.Position=UDim2.new(0.5,-w/2,0.5,-h/2) end) end
...
do local speedRow=Instance.new("Frame",SetFrame); speedRow.Size=UDim2.new(1,0,0,50); speedRow.BackgroundColor3=CARD; speedRow.BorderSizePixel=0; speedRow.LayoutOrder=2; Instance.new("UICorner",speedRow).CornerRadius=UDim.new(0,10); local speedBtn=Instance.new("TextButton",speedRow); speedBtn.Size=UDim2.new(0.85,-20,0,36); speedBtn.Position=UDim2.new(0.075,0,0.5,-18); speedBtn.BackgroundColor3=Color3.fromRGB(15,10,28); speedBtn.Text="Speed Customizer"; speedBtn.TextColor3=ACCENT; speedBtn.Font=Enum.Font.GothamBold; speedBtn.TextSize=13; speedBtn.BorderSizePixel=0; Instance.new("UICorner",speedBtn).CornerRadius=UDim.new(0,8); Instance.new("UIStroke",speedBtn).Color=ACCENT; local open=false; speedBtn.MouseButton1Click:Connect(function() if open then if S7Speed.SpeedUI and S7Speed.SpeedUI.ScreenGui then if S7Speed.Enabled then disableS7Speed() end; S7Speed.SpeedUI.ScreenGui:Destroy(); S7Speed.SpeedUI=nil end; open=false else createS7SpeedUI(); open=true end end) end
...
makeSection(BindFrame,"  KEYBINDS",1)
...
applyFOV()
print("S7 Hub loaded - discord.gg/5FaWfEvmJ")
